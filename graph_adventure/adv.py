...
# on every move, add path to traversalPath
...
def dft(player):
    ...
    while s.size() > 0:
        ...
        if current_room.id not in visited:
            ...
            # print(f'DFT In room: {current_room.id}')
            ...
            # check length of stack before the if and after the if
            # if it has the same length, run BFT
            if stack_before == stack_after:
                # BFT will return shortest PATH to the next non-visited room
                shortest_path = []
                try:
                    # if current_room.id == s.stack[-1].id (you hit a dead end in a looped nodes)
...
def bfs(start, target, dft_stack):
    ...
    while que.size() > 0:
        ...
        # print(f'*** BFS In room: {current_room["node"].id} ***')
        if current_room["node"].id not in bfs_visited:
            ...
            if current_room["node"].id == target.id:
                ...
                # print(f'BFS path: {current_room["path"]}')
